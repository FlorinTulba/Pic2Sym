...
class TestSymFilter {
  ...
  */
  static bool testCategory(const bool disposableCateg,
    ...
    Mat consec{1, (int)Settings_MAX_FONT_SIZE, CV_64FC1, 0.};
    Mat revConsec;
    ...
    for (unsigned symIdx{}; symIdx < symsCount; ++symIdx) {
      ...
      auto pms = std::make_unique<const PixMapSym>(
          symData,

          // Take only rows values from [rev]consec
          consec.colRange(Range{0, (int)rows}),
          revConsec.rowRange(Range{int(Settings_MAX_FONT_SIZE - rows),
                                   (int)Settings_MAX_FONT_SIZE}));

      const bool isDisposable{filteringEnabled &&
                              SymFilterType::isDisposable(*pms, sfc)};
  ...
  */
  static bool against(const path& pathPrefix) {
    ...
    if (!exists(positives) || !exists(negatives))
      reportAndThrow<invalid_argument>("Couldn't find "s + positives.string() +
                                       " or "s + negatives.string());
...
#define SAVE_PREV_ENABLED_STATE(FilterType) \
  bool old##FilterType##Enabled{FilterType##Enabled};
