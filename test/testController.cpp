...
class ControllerFixt : public Fixt {
 ...
 private:
  /// Tests shouldn't touch the settings; Keep it above ::Controller c
  Settings s;
...
TITLED_AUTO_TEST_CASE(ProvidingAnImageToController_SetColorImage_OkToSetImage) {
  Mat testColorPatch(c.getFontSize(), c.getFontSize(), CV_8UC3,
                     Scalar::all(127));
...
TITLED_AUTO_TEST_CASE(AttemptTransformation_SetGrayImage_OkToTransformImage) {
  ...
  cpa.newUnderGlyphCorrectnessFactor(
      1.);  // enable just one random aspect, to avoid missing enabled aspects
