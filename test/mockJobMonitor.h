...
class JobMonitor : public AbsJobMonitor {
 public:
  JobMonitor() noexcept : AbsJobMonitor("") {}
  JobMonitor(const std::string&,
             std::unique_ptr<IProgressNotifier>,
             double) noexcept
      : AbsJobMonitor("") {}

  void setTasksDetails(const std::vector<double>&, ITimerResult&) override {}
  unsigned monitorNewTask(AbsTaskMonitor&) noexcept override { return 0U; }
