...
class TestClusterEngine : public ClusterEngine {
 public:
  ...
  ~TestClusterEngine() noexcept { delete tsp; }
...
const unsigned
    TinySymMidSide =
        TinySymsSize >>
        1,  // TinySymsSize is odd, so it's ok to keep center as unsigned value
...
const SymData EmptySymData5x5(
    ...
    {{(int)ISymData::MaskType::GroundedSym,
      Mat(TinySymsSize, TinySymsSize, CV_64FC1, Scalar(0.))}},
...
const TinySym& MainDiagTinySym() {
  static const TinySym ts(
      255. - 255. * Mat::eye(TinySymsSize, TinySymsSize, CV_64FC1),
      Point2d(.5, .5), .2);
  return ts;
}
...
void updateCentralPixel(TinySym& sym, double pixelVal) {
  const double oldPixelVal = sym.mat.at<double>(TinySymMidSide, TinySymMidSide),
               ...
               diff = pixelVal - oldPixelVal, avgDiff = diff / TinySymsSize;
...
FIXTURE_TEST_SUITE_SUFFIX(SuiteFixture,
...
FIXTURE_TEST_SUITE_SUFFIX(SuiteFixture, BasicClustering_Tests, SuiteSuffix)
...
TITLED_AUTO_TEST_CASE_(UsingPartitionClustering_x0x0xSequence_2Clusters,
  ...
  BOOST_REQUIRE(clusters[0]->getIdxOfFirstSym() == 0 &&
                clusters[0]->getSz() ==
  ...
  BOOST_REQUIRE(clusterOffsets.find(0U) !=
                clusterOffsets.end());  // largest cluster starts at 0 (3 items)
  BOOST_REQUIRE(
      clusterOffsets.find(3U) !=
      clusterOffsets.end());  // smallest cluster starts at 3 (2 items)
  BOOST_REQUIRE(clusterOffsets.find((unsigned)symsCount) !=
                clusterOffsets.end());
  TITLED_AUTO_TEST_CASE_END
}
...
FIXTURE_TEST_SUITE_SUFFIX(SuiteFixture, TTSAS_Clustering_Tests, SuiteSuffix)
TITLED_AUTO_TEST_CASE_(
  ...
  for (unsigned n = 1U; n < 30U; ++n) {
    ...
    TinySym newSym =
        EmptyTinySym();  // changing only central pixel => same mass-center
    ...
    const double nPlus1 = (double)n + 1., nPlus1Sq = nPlus1 * nPlus1,
                 CentralPixelOfCentroidNplus1 =
                     TTSAS_Threshold_Member / nPlus1Sq,
                 ThresholdForNplus2 = TTSAS_Threshold_Member / ((double)n + 2.),
                 BorderMemberNplus2 =
                     CentralPixelOfCentroidNplus1 + ThresholdForNplus2;
