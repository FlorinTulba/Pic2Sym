...
StructuralSimilarity::StructuralSimilarity(const IMatchSettings& ms) noexcept
...
void MatchParams::computeSsim(const Mat& patch,
  ...
  static const bool usingGaussianBlur =
      (StructuralSimilarity_BlurType == "gaussian");
#endif  // _DEBUG
  ...
  const Mat numerator = (2. * productBlurredMats + StructuralSimilarity_C1)
            ...
            denominator = (blurredPatchSq.value() + blurredPatchApproxSq +
                           StructuralSimilarity_C1)
