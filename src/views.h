...
class CvWin /*abstract*/ : public virtual ICvWin {
 public:
  ...
  void setOverlay(const std::string& overlay,
                  int timeoutMs = 0) const noexcept override;
 ...
 private:
  cv::String _winName;  ///< window's handle
...
class Comparator : public CvWin, public virtual IComparator {
 ...
 private:
  ...
  int trackPos{};           ///< Transparency value
};
...
class CmapInspect : public CvWin, public virtual ICmapInspect {
 public:
  ...
  void showUnofficial1stPage(
      const std::vector<cv::Mat>& symsOn1stPage,
 ...
 protected:
  ...
  void populateGrid(const ICmapPerspective::VPSymDataRange& itPair,
                    const std::set<unsigned>& clusterOffsets,
                    unsigned idxOfFirstSymFromPage) noexcept;

 private:
  gsl::not_null<const IPresentCmap*>
      cmapPresenter;  ///< presents the cmap window

  cv::Mat grid;                           ///< the symbols' `hive`
  int page{};                             ///< page slider position
  unsigned pagesCount{};                  ///< used for dividing the cmap
  gsl::not_null<const unsigned*> fontSz;  ///< font size
  unsigned cellSide{};                    ///< used for dividing the cmap
  unsigned symsPerRow{};                  ///< used for dividing the cmap
  unsigned symsPerPage{};                 ///< used for dividing the cmap
