...
static bool acceptableProfile(const Mat &narrowGlyph,	///< bounding box (BB) region of the glyph
	...
	for(lastNonEmptyRowColBB = lastRowColBB, rowColProj = lastNonEmptyRowColBB + firstRowColBB;
		0.f == projSums.at<fp>((int)rowColProj); --rowColProj, --lastNonEmptyRowColBB) {}
	for(firstNonEmptyRowColBB = 0U, rowColProj = firstRowColBB;
		0.f == projSums.at<fp>((int)rowColProj); ++rowColProj, ++firstNonEmptyRowColBB) {}

#pragma warning ( disable : WARN_THREAD_UNSAFE )
	static const fp FactorTolUnder = .9f, FactorTolAbove = 1.f / FactorTolUnder;
...
bool GridBarsFilter::checkProjectionForGridSymbols(const Mat &sums) {
	const auto itFirst = sums.begin<fp>(), itEnd = sums.end<fp>();
	set<fp> uniqueVals(itFirst, itEnd);
	if(uniqueVals.size() == 1ULL)
		return true; // Pattern is: A+   (case (c))

	uniqueVals.erase(0.f); // Erase the 0, if present

	auto itUniqueVals = uniqueVals.crbegin(); // reverse iterator !!
	fp A = *itUniqueVals++, t1 = -1.f, t2 = -1.f;
	...
	for(auto val : vals) {
		switch(state) {
			case Read0s:
				if(val == 0.f) continue; // stay in state Read0s
			...
			case ReadT1:
				...
				if(val == A) {
					...
					if(!metA) return false; // case (a) A is expected here
					if(t2 != -1.f) return false;  // case (b) t2 is expected here
					state = Read0s;
				}
				break;
			...
			case ReadA:
				...
				if(val == 0.f) { state = Read0s; } else { state = ReadT1; }
				break;
...
#pragma warning ( default : WARN_THREAD_UNSAFE )
	...
	Mat projSum;
	closedGlyphBin.convertTo(closedGlyphBin, CV_FC1, INV_255());
