...
#pragma warning ( default : WARN_THREAD_UNSAFE )
	...
	for(unsigned rc = firstNonEmptyRowColBB + 1U; rc <= lastNonEmptyRowColBB; ++rc) {
		...
		if(cnzPrevData == cnzThisRowCol) {
			MatConstIterator_<unsigned char> itThisRowCol = thisRowCol.begin<unsigned char>(),
				itThisRowColEnd = thisRowCol.end<unsigned char>();
			MatConstIterator_<unsigned char> itPrevDataInfLim = prevDataInfLim.begin<unsigned char>(),
				itPrevDataSupLim = prevDataSupLim.begin<unsigned char>();
			bool quiteTheSamePattern = true;
			while(itThisRowCol != itThisRowColEnd) {
				const unsigned char cellVal = *itThisRowCol;
...
GridBarsFilter::GridBarsFilter(uniquePtr<ISymFilter> nextFilter_/* = nullptr*/) :
...
bool GridBarsFilter::checkProjectionForGridSymbols(const Mat &sums) {
	const MatConstIterator_<double> itFirst = sums.begin<double>(), itEnd = sums.end<double>();
