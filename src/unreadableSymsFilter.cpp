...
SYM_FILTER_DEFINE_IS_ENABLED(UnreadableSymsFilter)
...
bool UnreadableSymsFilter::isDisposable(const IPixMapSym& pms,
  ...
  // masks need to be larger for larger fonts
  const int winSideSE{(sfc.getSzU() > 15U) ? 5 : 3};
  const Size winSE{winSideSE, winSideSE};

  Mat glyph{pms.toMatD01(sfc.getSzU())};
  Mat thresh, glyphBinAux;
  ...
  Mat glyphBin{frameSz, frameSz, CV_8UC1, Scalar{}};
  Mat depth;
  ...
  // just below 2 (2 means there are sections at least 3 pixels thick)
  double maxValAllowed{1.9};
  if (sfc.getSzU() > 10U)
    maxValAllowed =
        sfc.getSzU() /
