...
/*
  ...
  It was included in the project since it also presents a working version for
  CUDA that provides great time-performance improvement. Credits for this CUDA
  version to Michael <lioucr@hotmail.com> - http://home.so-net.net.tw/lioucy
  */
...
#pragma warning(disable : WARN_THROWS_ALTHOUGH_NOEXCEPT)
  ...
  @throw invalid_argument if desiredSigma <= 0

  Exception to be only reported, not handled. It propagates only in UnitTesting
  */
  Impl& setSigma(double desiredSigma) noexcept(!UT) {
    if (desiredSigma <= 0.)
...
#pragma warning(disable : WARN_THROWS_ALTHOUGH_NOEXCEPT)
  ...
  @throw invalid_argument if radius outside 1..254

  Exception to be only reported, not handled. It propagates only in UnitTesting
  */
  Impl& setRadius(unsigned radius) noexcept(!UT) {
    if (radius == 0U || radius > 254U)
      THROW_WITH_CONST_MSG(__FUNCTION__ " needs radius  in range 1..254!",
                           invalid_argument);
    r = radius;
...
#pragma warning(default : WARN_THROWS_ALTHOUGH_NOEXCEPT)
  ...
  /**
  Implementation of the Stack Blur Algorithm by Mario Klingemann
  (<mario@quasimondo.com>) for single channel images with pixel of type double.
  ...
  - used a single scaling / result pixel instead of 2:
    Resulted pixel gets multiplied only during the processing of the columns
  with mul_sum_sq = mul_sum^2, and not twice with mul_sum during each traversal.
  */
  void apply(const cv::Mat& toBlur, cv::Mat& blurred) const noexcept {
    double* const pResult = (double*)blurred.data;
    const double* const pToProcess =
        (double*)toBlur.data;  // after processing the rows, it will be
                               // reassigned with a const_cast
    const unsigned w = (unsigned)toBlur.cols, h = (unsigned)toBlur.rows,
                   stride = (unsigned)toBlur.step1(), wm1 = w - 1U,
                   hm1 = h - 1U, rp1 = r + 1U, div = (r << 1) | 1U,
                   xp0 = min(wm1, r), yp0 = min(hm1, r);
    const double rp2 = r + 2.,
                 mul_sum = stack_blur8_mul[(size_t)r] /
                           pow(2, stack_blur8_shr[(size_t)r]),
                 mul_sum_sq = mul_sum * mul_sum;  // multiply only once, during
                                                  // the processing of columns

    unsigned x = 0U, y = 0U, xp = 0U, yp = 0U, i = 0U, stack_ptr = 0U,
             stack_start = 0U;

    const double* src_pix_ptr = nullptr;
    double *dst_pix_ptr = nullptr, *stack_pix_ptr = nullptr;
    double* const stack = new double[(size_t)div];
    double sumT = 0., sumIn = 0., sumOut = 0.;

    y = 0U;
    do {  // Process image rows, this outer while-loop will be parallel computed
          // by CUDA instead
      // Get input and output weights
      const unsigned row_addr = y * stride;
      double pix = sumT = sumOut = stack[0ULL] =
          *(src_pix_ptr = pToProcess + row_addr);
      sumIn = 0.;
      for (i = 1U; i <= xp0; ++i) {
        stack[(size_t)i] = pix;
        sumT += pix * (i + 1.);
      ...
      if (i <= r) {  // for a radius larger than image width
        ...
        sumIn += total;
        sumT += rp2 * total;
        fill(stack + i, stack + rp1, pix);
        fill(stack + i + r, stack + div, pix);
      }

      stack_ptr = r;
      src_pix_ptr = pToProcess + ((xp = xp0) + (size_t)row_addr);
      dst_pix_ptr = pResult + row_addr;

      x = 0U;
      do {  // Blur image rows
        *dst_pix_ptr++ =
            sumT /* * mul_sum*/;  // multiply only once with its square, during
                                  // the processing of columns
        sumT -= sumOut;
    ...
    } while (++y < h);    // Process image rows, this outer while-loop will be
                          // parallel computed by CUDA instead
...
#pragma warning(default : WARN_LVALUE_CAST)
    x = 0U;
    do {  // Process image columns, this outer while-loop will be parallel
          // computed by CUDA instead
      double pix = sumT = sumOut = stack[0ULL] =
          *(src_pix_ptr = pToProcess + x);
      sumIn = 0.;
      for (i = 1U; i <= yp0; ++i) {
        stack[(size_t)i] = pix;  // code analysis signals buffer overrun here,
                                 // but 2 lines below doesn't, altough r>=0 !!!
        sumT += pix * (i + 1.);
        sumOut += pix;
      ...
      stack_ptr = r;
      src_pix_ptr = pToProcess + (x + ((yp = yp0) * (size_t)stride));
      ...
      do {  // Blur image columns
        *dst_pix_ptr =
            sumT /* * mul_sum*/ *
            mul_sum_sq;  // multiply only once with the square of mul_sum
        dst_pix_ptr += stride;
    ...
    } while (++x < w);  // Process image columns, this outer while-loop will be
                        // parallel computed by CUDA instead
...
StackBlur::Impl& StackBlur::nonTinySyms() noexcept {
  return StackBlur::Impl::_nonTinySyms;
}

StackBlur::Impl& StackBlur::tinySyms() noexcept {
  return StackBlur::Impl::_tinySyms;
}

void StackBlur::doProcess(const cv::Mat& toBlur,
                          cv::Mat& blurred,
...
StackBlur::StackBlur(unsigned radius) noexcept(!UT) {
  setRadius(radius);
}

StackBlur& StackBlur::setSigma(double desiredSigma) noexcept(!UT) {
...
StackBlur& StackBlur::setRadius(unsigned radius) noexcept(!UT) {
...
const StackBlur& StackBlur::configuredInstance() noexcept(!UT) {
