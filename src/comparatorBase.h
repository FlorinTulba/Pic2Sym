...
/**
Interface of a view which permits comparing the original image with the transformed one.

A slider adjusts the transparency of the resulted image,
so that the original can be more or less visible.
*/
struct IComparator /*abstract*/ : virtual ICvWin {
	virtual void setReference(const cv::Mat &reference_) = 0;
	virtual void setResult(const cv::Mat &result_,
						   int transparency =
								(int)round(Comparator_defaultTransparency * Comparator_trackMax)) = 0;
