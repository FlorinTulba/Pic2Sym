...
class IFontEngine /*abstract*/ : public ITinySymsProvider {
  ...
  */
  virtual void setFontSz(unsigned fontSz_) noexcept(!UT) = 0;
  ...
  */
  ...
  Exception to be only reported, not handled
  */
  virtual const VPixMapSym& symsSet() const noexcept(!UT) = 0;
  ...
  */
  ...
  virtual IFontEngine& useSymsMonitor(AbsJobMonitor& symsMonitor_) noexcept = 0;

 protected:
  IFontEngine() noexcept {}
};
