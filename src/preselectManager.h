...
class IPreselManager /*abstract*/ {
 public:
  ...
  virtual std::unique_ptr<IClustersSupport> createClusterSupport(
      ITinySymsProvider& tsp,
      IClusterProcessing& ce,
      VSymData& symsSet) const noexcept = 0;

  virtual std::unique_ptr<IMatchSupport> createMatchSupport(
      CachedDataRW& cd,
      VSymData& symsSet,
      MatchAssessor& matchAssessor,
      const IMatchSettings& matchSettings) const noexcept = 0;
  ...
  virtual std::unique_ptr<ITransformSupport> createTransformSupport(
      ...
      IMatchSupport& matchSupport) const noexcept = 0;

  virtual ~IPreselManager() noexcept {}

  // Slicing prevention
  IPreselManager(const IPreselManager&) = delete;
  IPreselManager(IPreselManager&&) = delete;
  IPreselManager& operator=(const IPreselManager&) = delete;
  IPreselManager& operator=(IPreselManager&&) = delete;

 protected:
  constexpr IPreselManager() noexcept {}
};
