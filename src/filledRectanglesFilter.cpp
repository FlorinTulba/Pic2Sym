...
@param sums the projection (reduction sum) of the glyph in horizontal / vertical
direction
@param sideLen height / width of glyph's bounding box
@param countOfMaxSums [out] determined side length of a possible rectangle:
[1..sideLen]
...
bool FilledRectanglesFilter::checkProjectionForFilledRectangles(
  ...
  const Mat sumsOnMax =
      (sums == maxVSums);  // these should be the white rows/columns
  ...
  if (countOfMaxSums == 2) {
    ...
    if (countNonZero(paddedSumsOnMax) - countOfMaxSums > 2)
      return false;  // there was at least one gap, so dilation filled more than
                     // 2 pixels
  } else {           // countOfMaxSums is [3..sideLen)
    erode(sumsOnMax, sumsOnMax, structuringElem);
    if (countOfMaxSums - countNonZero(sumsOnMax) > 2)
      return false;  // there was at least one gap, so erosion teared down more
                     // than 2 pixels
  }
  return true;
}
...
bool FilledRectanglesFilter::isDisposable(const IPixMapSym& pms,
  ...
  const int brightestPixels =
      countNonZero(narrowGlyph == (unsigned char)brightestVal);
  if (brightestPixels < 3)
    return false;  // don't report really small areas

  // Sides of a possible filled rectangle
  int rowsWithMaxSum = 0, colsWithMaxSum = 0;

  // Analyze the horizontal and vertical projections of pms, looking for signs
  // of rectangle symbols
