...
class MatchAspect /*abstract*/ {
 public:
  ...
  MatchAspect(MatchAspect&&) noexcept = default;
  ...
  double maxScore(const CachedData& cachedData) const noexcept;
 ...
 protected:
  ...
  /// MatchParams mp
  virtual double score(const IMatchParams& mp,
                       const CachedData& cachedData) const noexcept = 0;
...
(1) Create a class for it using the template:
  ...
  PROTECTED: // UNIT_TESTING needs the constructors as public
    ...
    /// Constructor Details
    explicit <NewAspect>(const IMatchSettings &ms) noexcept;
